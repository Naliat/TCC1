...
def compute_score_alg1(mod_int: int, notas: tuple, mod_vaga_int: int):
    prio = _PRIO_MATRIX[mod_vaga_int, mod_int] if mod_int >= 0 else 999
    return (-prio, notas)
def elegivel_para_vaga(cand_idx: int, opcao: int, id_curso_vaga: int, mod_vaga_int: int,
    ...
    return False
def build_all_propostas_sisu(cands: CandidatosArray, elegibilidade_base: list) -> list:
    _pos_global = {m: i for i, m in enumerate(ORDEM_GLOBAL_SISU)}
    propostas_todas = []
    for idx in range(cands.n):
        ...
        for opcao, elegiveis in enumerate(elegibilidade_base[idx]):
            if not elegiveis:
                continue
            elegiveis_ord = sorted(
                elegiveis,
                key=lambda cm: _pos_global.get(_INT_TO_MOD.get(cm[1], ''), 999)
            )
            lista.extend(elegiveis_ord)
        propostas_todas.append(lista)
    return propostas_todas
def calcular_deficits(vagas_dict: dict, alocacoes: dict) -> dict:
    deficits = {}
    for chave, cap in vagas_dict.items():
        ocupacao = len(alocacoes.get(chave, []))
        deficits[chave] = cap - ocupacao
    return deficits
def build_all_propostas_odf(cands: CandidatosArray, elegibilidade_base: list,
                             deficits: dict) -> list:
    _ordem_sisu = {m: i for i, m in enumerate(ORDEM_GLOBAL_SISU)}
    propostas_todas = []
    for idx in range(cands.n):
        lista = []
        for opcao, elegiveis in enumerate(elegibilidade_base[idx]):
            ...
            mod_proprio = int(cands.modalidade[idx, opcao])
            propria = [(id_curso, mod_proprio)] if (id_curso, mod_proprio) in elegiveis else []
            remanejo = [
                cm for cm in elegiveis
                if cm[1] != AC_INT and cm[1] != mod_proprio
            ]
            remanejo.sort(key=lambda cm: (
                -deficits.get(cm, 0),
                _ordem_sisu.get(_INT_TO_MOD.get(cm[1], ''), 999)
            ))

            lista.extend(propria)
            lista.extend(remanejo)
            if mod_proprio != AC_INT:
...
def main():
    print("=" * 70)
    print("ALGORITMO ODF — Observed Deficit First")
    print("=" * 70)
    ...
    print("\nPASSO 4: Executando alocação SISU baseline (para medir déficits)...")
    t0 = time.time()
    propostas_sisu = build_all_propostas_sisu(cands, elegibilidade_base)
    _, _, aloc_sisu = gale_shapley(
        cands, vagas_dict, propostas_sisu, compute_score_alg1, verbose=False
    )
    deficits  = calcular_deficits(vagas_dict, aloc_sisu)
    n_ociosas = sum(1 for d in deficits.values() if d > 0)
    print(f"   Déficits do SISU: {n_ociosas:,} vagas ociosas "
          f"({n_ociosas / len(vagas_dict) * 100:.1f}% das combinações)")
    print(f"   {time.time() - t0:.2f}s")

    print("\n" + "=" * 70)
    print("EXECUTANDO ALGORITMO ODF")
    t1 = time.time()
    propostas_odf = build_all_propostas_odf(cands, elegibilidade_base, deficits)
    total, metricas, alocacoes = gale_shapley(
        cands, vagas_dict, propostas_odf, compute_score_alg1, verbose=VERBOSE
    )
    t1 = time.time() - t1
    ...
    df_aloc.to_csv('alocados_ODF.csv', index=False, encoding='utf-8-sig')
    print(f"   alocados_ODF.csv: {len(df_aloc):,} linhas")
