...
def main():
    print("=" * 70)
    print("ALGORITMO SISU — Baseline determinístico (ordem oficial do edital)")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    print("EXECUTANDO ALGORITMO SISU")
    t1 = time.time()
    propostas = build_all_propostas_sisu(cands, elegibilidade_base)
    total, metricas, alocacoes = gale_shapley(
        cands, vagas_dict, propostas, compute_score_alg1, verbose=VERBOSE
    )
    t1 = time.time() - t1
    ...
    df_aloc.to_csv('alocados_SISU.csv', index=False, encoding='utf-8-sig')
    print(f"   alocados_SISU.csv: {len(df_aloc):,} linhas")
